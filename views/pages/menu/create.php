<?php /** @var string $error, $csrf, $title */ ?>
<div class="mb-6 flex items-center gap-3">
  <a href="/menu" class="text-gray-400 hover:text-navy-700 text-sm">← Retour</a>
  <h1 class="text-2xl font-bold text-navy-900">Nouvel article</h1>
</div>
...
<form method="POST" action="/menu/create" enctype="multipart/form-data" class="grid lg:grid-cols-3 gap-6">
  ...
  <div class="lg:col-span-2 bg-white rounded-2xl p-6 shadow-sm border border-gray-100 space-y-4">
    <div>
      ...
      <input type="text" name="name" required placeholder="Ex: Petit-déjeuner continental"
    ...
    <div>
      ...
      <textarea name="description" rows="3" placeholder="Ingrédients, allergènes, etc."
                class="w-full px-3 py-2.5 rounded-lg border border-gray-300 text-sm focus:ring-2 focus:ring-navy-700 focus:border-transparent outline-none"></textarea>
    </div>
    <div class="grid sm:grid-cols-3 gap-4">
      <div>
        <label class="block text-xs font-medium text-gray-600 mb-1">Prix (FCFA) *</label>
        <input type="number" name="price" min="0" step="0.01" required placeholder="5000"
               class="w-full px-3 py-2.5 rounded-lg border border-gray-300 text-sm focus:ring-2 focus:ring-navy-700 focus:border-transparent outline-none">
      </div>
      <div>
      ...
      <div>
        ...
        <input type="number" name="prep_time" min="0" step="1" value="15"
               class="w-full px-3 py-2.5 rounded-lg border border-gray-300 text-sm focus:ring-2 focus:ring-navy-700 focus:border-transparent outline-none">
      </div>
    </div>
    <label class="flex items-center gap-2 cursor-pointer">
      <input type="checkbox" name="available" value="1" checked class="w-4 h-4 accent-navy-700">
  ...
  <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
    ...
    <div class="border-2 border-dashed border-gray-200 rounded-xl p-4 text-center">
      ...
      <p class="text-[10px] text-gray-400 mt-2">jpg, png, webp · max 2 Mo</p>
    </div>
    <button type="submit" class="w-full mt-6 py-2.5 rounded-lg bg-gold-400 text-white text-sm font-semibold hover:bg-gold-500 transition">Créer l'article</button>
  </div>
</form>
