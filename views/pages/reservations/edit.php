<?php /** @var array $reservation, $guests, $rooms, $form, $errors, $sources, $statuses, $title */ ?>
<div class="mb-6">
  <div class="flex items-center gap-2 text-xs text-gray-400 mb-1">
    <a href="/reservations" class="hover:text-navy-700">Réservations</a>
    <span>/</span>
    <span>Modifier #<?= (int) $reservation['id'] ?></span>
  </div>
  <h1 class="text-2xl font-bold text-navy-900">Modifier la réservation</h1>
</div>
...
<form method="POST" action="/reservations/edit/<?= (int) $reservation['id'] ?>" class="grid lg:grid-cols-3 gap-6">
    ...
    <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
      ...
      <div class="grid sm:grid-cols-2 gap-4">
        <div>
          ...
          <select name="guest_id" class="w-full px-4 py-2 rounded-lg border border-gray-300 text-sm focus:ring-2 focus:ring-navy-700 focus:border-navy-700 outline-none">
        ...
        <div>
          ...
          <select name="room_id" id="room_id" class="w-full px-4 py-2 rounded-lg border border-gray-300 text-sm focus:ring-2 focus:ring-navy-700 focus:border-navy-700 outline-none" onchange="updateTotal()">
            <?php foreach ($rooms as $rm): ?>
              <option value="<?= (int) $rm['id'] ?>" <?= (int) $form['room_id'] === (int) $rm['id'] ? 'selected' : '' ?>>
                N° <?= e($rm['number']) ?> · <?= e($rm['type_name'] ?? 'Sans type') ?> · Ét. <?= (int) $rm['floor'] ?>
              </option>
            <?php endforeach; ?>
          </select>
        </div>
  ...
  <div class="space-y-6">
    <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 sticky top-6">
      ...
      <a href="/reservations/view/<?= (int) $reservation['id'] ?>" class="mt-2 block text-center text-sm text-gray-500 hover:text-navy-700">Annuler</a>
    </div>
  </div>
